...
def transform_raw_input_consumos(df):
    ...
    df = df[['mesfac', 'numcta', 'instalacion', 'ciclo', 'sector', 'ruta', 'manzana', 'secuencia', 'categoria',
        'consumo_medido', 'metodo_consumo', 'cl_inst', 'cod_hidraulico', 'year', 'mes', 'date']]
...
def concatenar_nuevos_datos_consumo():
    ...
    df_historico = pd.concat([df_historico,df_consumo])
    # RECONVERTIR A LOS TIPOS DE DATOS ORIGINALES STRING Y CATEGORY
    cols_string = ['mesfac', 'numcta', 'instalacion', 'year', 'mes']
...
def series_de_consumo_etiquetadas(df_fraudes_completo_etiquetado):
...
def cleaning_feature_engineering(df_wide_completo):
...
def feature_selection_dataset_entrenamiento(df):
...
def transform_raw_input_inspecciones(df_inspecciones, df_multas, df_instalacion_cuenta):
    '''
...
def concatenar_nuevos_datos_inspecciones():
    ...
    if len(df_inspecciones)>0 and len(df_multas)>0:
        df_inspecciones = transform_raw_input_inspecciones(df_inspecciones, df_multas, df_map_inst_cuenta)
...
def etiquetar_consumos_inspecciones_train(df, df_fraudes):
    '''
    CRUZAR DATOS DE CONSUMO CON INSPECCIONES REALIZADAS, PARA ETIQUETAR LOS CONSUMOS EN LAS CUENTAS INSPECCIONADAS
    GENERA DATASET DE CONSUMOS, CON SUS ETIQUETAS DE FRAUDE O NO FRAUDE (is_fraud)
    '''
...
def series_de_consumo_mes_especifico(df_consumo,mes=False):
    ...
    df_series_final = df_series_final.merge(df_categoria.drop(columns=['categoria']), on='instalacion')
...
def cleaning_feature_engineering_mes_especifico(df_series_mes):
    ...
    df_features = pipe_feature_engeniering_consumo.fit_transform(
        df_series_mes[['index']+variables_consumo])
    cols_fs = ['index']+df_features.columns.tolist()[len(variables_consumo)+1:]
    df_completo = df_series_mes.merge(df_features[cols_fs], how='inner', on=['index'], indicator=False)
    df_completo.to_parquet(nombre_path_historico_series_features_mes)
    print(f'Tamaño final series con features del mes: {df_completo.shape}')
